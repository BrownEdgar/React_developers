...
export default function usersReducer(state = initialUeersValue, action) {
	switch (action.type) {
		...
		case DELETE_BY_ID: return {
			...state, 
			users: state.users.filter(user => user.id !== action.payload.id)
		};

		case DELETE_BY_NAME: return {
			...state, 
		...
		case USER_BY_LOGIN: return {...state, result: state.users.find(user => user.login === action.payload.login) || 'not found'};
